...
- Methods are similar to functions which are defined with fn keyword and name. Unlike functions, methods are defined within the context of a struct and the first param is always self, which represents the instance of the struct the method is being called on. 
...
- To define the function within the context of the Rectangle , we start implementation block for rectangle. Everything in impl block will be associated with the Rectangle type.
